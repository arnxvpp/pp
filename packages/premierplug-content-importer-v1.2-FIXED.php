<?php
/**
 * PremierPlug Content Importer v1.2 - FIXED
 *
 * Imports all HTML content to WordPress with CLEAN HTML
 * Strips Drupal classes and inline styles
 * Creates clean WordPress-compatible content
 *
 * FIXES:
 * - Removes all Drupal-specific CSS classes
 * - Removes all inline styles
 * - Removes wrapper divs that break layout
 * - Keeps only semantic HTML (p, h1-h6, ul, ol, img, etc.)
 *
 * Usage: Upload to WordPress root and visit: your-site.com/premierplug-content-importer-v1.2-FIXED.php?key=premierplug_import_2024
 */
...
<head>
    <meta charset="utf-8">
    <title>PremierPlug Content Importer v1.2 - FIXED</title>
    <style>
        body {
...
<body>
<div class="container">
    <h1>🚀 PremierPlug Content Importer v1.2 - FIXED</h1>

    <div class="status warning">
        <strong>⚠️ IMPORTANT:</strong> This version CLEANS the HTML to prevent CSS conflicts.<br>
        • Removes all Drupal CSS classes<br>
        • Removes inline styles<br>
        • Creates clean WordPress-compatible content<br>
        • Preserves only semantic HTML (headings, paragraphs, lists, images)
    </div>

<?php

// Configuration
$archive_path = __DIR__ . '/archive/old-site/archive/';
$images_path = __DIR__ . '/premierplug-theme/assets/images/';
...
function extract_content_from_html($html) {
    ...
    // Remove aria- attributes (keep aria-label for accessibility)
    $content = preg_replace('/ aria-(?!label)[a-z\-]+=("[^"]*"|\'[^\']*\')/', '', $content);

    // Remove role attributes
    $content = preg_replace('/ role="[^"]*"/', '', $content);

    // Remove title attributes on divs/spans
    $content = preg_replace('/<(div|span)[^>]* title="[^"]*"/', '<$1', $content);
...
function get_or_create_page($data, &$created_pages) {
    ...
    if ($existing) {
        ...
        $parent_id = 0;
        if ($data['parent'] && isset($created_pages[$data['parent']])) {
    ...
    // Load and parse HTML content with CLEAN extraction
    $content = '';
    $excerpt = '';
    if ($data['file']) {
        $html_file = $GLOBALS['archive_path'] . $data['file'];
...
// Run import
echo '<div class="status info"><strong>Starting import with CLEAN HTML...</strong></div>';
...
foreach ($pages_map as $index => $page_data) {
    ...
    if ($page_id) {
        ...
        echo '<td><span class="check">✓ Created/Updated</span></td>';
        echo '<td>ID: ' . $page_id;
        if ($page_data['parent']) {
        ...
        echo ' | <strong>HTML: CLEANED</strong>';
        echo '</td>';
        echo '</tr>';
    } else {
        $error_count++;
...
echo '✓ Pages created/updated: ' . $success_count . '<br>';
if ($error_count > 0) {
...
echo '<strong>✓ HTML CLEANED - All Drupal classes removed!</strong><br>';
echo '</div>';

echo '<div class="status info">';
echo '<strong>What Was Fixed:</strong><br>';
echo '✓ Removed all Drupal CSS classes (role--anonymous, layout-container, etc.)<br>';
echo '✓ Removed all inline styles<br>';
echo '✓ Removed all ID attributes<br>';
echo '✓ Removed wrapper divs<br>';
echo '✓ Created clean WordPress-compatible content<br>';
echo '✓ Your site CSS will now work properly!<br>';
echo '</div>';

echo '<div class="status warning">';
echo '<strong>Next Steps:</strong><br>';
echo '1. Visit your site: <a href="' . home_url() . '" target="_blank">' . home_url() . '</a><br>';
echo '2. Clear browser cache (Ctrl+Shift+R)<br>';
echo '3. Check all pages - should look correct now!<br>';
echo '4. Delete this import file for security<br>';
echo '</div>';
